...
//using SC;
//using DM;
public class NiisanController : NetworkBehaviour {
	...
	void Start () {
		...
		if (!bloodPos) {
			healthSlider = GameObject.Find ("BloodBarLeft").GetComponent<Slider> ();
			//GameObject.Find ("BloodBarRight").GetComponent<Slider>().fillRect.GetComponent<Image>().color = Color.yellow;
			//GameObject.Find ("BloodBarRight").GetComponent<Slider> ();
			//healthBar = GameObject.Find("LeftForeground").GetComponent<RectTransform>();
		} else {
			healthSlider = GameObject.Find ("BloodBarRight").GetComponent<Slider> ();
			//GameObject.Find ("BloodBarLeft").GetComponent<Slider>().fillRect.GetComponent<Image>().color = Color.yellow;
			//GameObject.Find ("BloodBarLeft").GetComponent<Slider> ();
			//healthBar = GameObject.Find("RightForeground").GetComponent<RectTransform>();
		}
    ...
    {
		if (enemy!=null && enemy!=gameObject && collision.gameObject==enemy && enemy.GetComponent<NiisanController>().GetLightningStatus() && !lightningStatus && !dead) 
		{
			/* collision.gameObject.name.Contains ("Enemy") && !StatusCheck.PlightningStatus && StatusCheck.AIlightningStatus */ 
			//health = health - 2;
			CmdHealth(-2);
			CmdHealthBar ();
			var combo = GetComponent<NiisanCombo> ();
			combo.CalcCombo ();
	...
	void Update () {
		...
		if (dead){
			var combo = GetComponent<NiisanCombo>();
			combo.GameOver(false);
			return;
		} else if(enemy != null && !enemy.GetComponent<NiisanController>().ifAlive()){
			var combo = GetComponent<NiisanCombo>();
			combo.GameOver(true);
		}
		...
		*/
		...
		if (m_CurrentClipInfo[0].clip.name == "LightningOn" && trigger && !dead) {
			...
			invincible = true;
			if (enemy!=null && enemy!=gameObject && enemy.GetComponent<NiisanController>().ifAlive()) {
	...
	int BeingHitCheck(){
		...
		if (enemy.GetComponent<NiisanController>().GetAttackType() == 1 && distance <= LIGHT_ATTACK)
		{
			return 1;
		} else if (enemy.GetComponent<NiisanController>().GetAttackType() == 2 && distance <= HEAVY_ATTACK) {
			return 2;
		}
		return 0;
	}
	...
	void RpcHealthBar(){
		...
		healthSlider.value = currentHealth;
		//healthBar.sizeDelta = new Vector2(currentHealth*2f, healthBar.sizeDelta.y);
	}
